    ...
    public @Nullable BlockState getToolModifiedState(BlockState state, UseOnContext context,
        ...
        if(context.getItemInHand().getItem() instanceof AxeItem) {
            if(state.is(ModBlocks.BLOODWOOD_LOG)) {
                ModBlocks.STRIPPED_BLOODWOOD_LOG.get().defaultBlockState().setValue(AXIS,state.getValue(AXIS));
            }

            if(state.is(ModBlocks.BLOODWOOD_WOOD)) {
                ModBlocks.STRIPPED_BLOODWOOD_WOOD.get().defaultBlockState().setValue(AXIS,state.getValue(AXIS));
            }
        }
